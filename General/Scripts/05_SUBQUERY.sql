...
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
GROUP BY DEPT_TITLE
HAVING SUM(SALARY) = (SELECT MAX(SUM(SALARY))	FROM EMPLOYEE	GROUP BY DEPT_CODE); -- 서브쿼리 결과 21760000
...
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
GROUP BY DEPT_TITLE
HAVING COUNT(*) = (SELECT MIN(COUNT(*))	FROM EMPLOYEE	GROUP BY DEPT_CODE); -- 서브쿼리 결과 : 2
...
SELECT 이름, 급여
FROM (SELECT EMP_NAME 이름, SALARY 급여 FROM EMPLOYEE)
WHERE 급여 >= 4000000;
...
FROM EMPLOYEE
WHERE MANAGER_ID IS NOT NULL; -- 7행 조회

-- 2) 직원의 사번, 이름, 부서명, 직급명 조회
SELECT EMP_ID, EMP_NAME, NVL(DEPT_TITLE, '없음') AS DEPT_TITLE, JOB_NAME
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);

-- 3) 사수에 해당하는 직원에 대한 정보 추출 조회 (이때, 구분은 '사수'로)
SELECT EMP_ID, EMP_NAME, NVL(DEPT_TITLE, '없음') AS DEPT_TITLE, JOB_NAME, '사수' AS "구분"
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
SELECT EMP_ID, EMP_NAME, NVL(DEPT_TITLE, '없음') AS DEPT_TITLE, JOB_NAME, '사원' AS "구분"
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
FROM EMPLOYEE
WHERE ENT_YN = 'Y' AND SUBSTR(EMP_NO, 8, 1) IN ('2', '4'); -- 이태림
		  -- 퇴사여부 = 'Y' == 퇴사한 직원
...
WHERE (DEPT_CODE, JOB_CODE) = (SELECT DEPT_CODE, JOB_CODE
									 						 FROM EMPLOYEE
									 						 WHERE ENT_YN = 'Y' AND SUBSTR(EMP_NO, 8, 1) IN ('2', '4'));
--> 여러 컬럼을 묶어서 한 번에 비교해 모두 같은 행만 조회 결과(RESULT SET)에 포함시킴
...
-- 6. 스칼라 서브쿼리(== SELECT절에서 사용되는 단일행 서브쿼리)
...
SELECT 
	...
	(SELECT AVG(SALARY)
	 FROM EMPLOYEE "SUB"
	 WHERE "SUB".JOB_CODE = "MAIN".JOB_CODE) AS "직급별 급여 평균"
FROM EMPLOYEE "MAIN"
ORDER BY EMP_ID ASC;
 ...
 * 	-> SELECT는 가능하지만 INSERT, UPDATE, DELETE는 불가능함
 */
...
WHERE ROWNUM <= 5
ORDER BY SALARY DESC;
--> 조회 결과가 잘못됨!!! --> EMPLOYEE 테이블 위에서 5명만 조회, 정렬된 결과
...
SELECT ROWNUM "순위", "부서 코드", "부서명", "급여 평균"
FROM
...
AS (SELECT EMP_NAME, SALARY -- 저장할 서브쿼리 작성
	  FROM EMPLOYEE
	  ORDER BY SALARY DESC)
...
SELECT RANK() OVER(ORDER BY SALARY DESC) "순위", EMP_NAME, SALARY
			 --> SALARY 내림차순으로 정렬하고 그 순위를 지정함
FROM EMPLOYEE;

-- 2) DENSE_RANK() OVER
--		: 공동 순위 저정 후 순위 건너뛰기를 하지 않음
...
FROM
	(SELECT RANK() OVER(ORDER BY SALARY DESC) "순위", EMP_NAME, SALARY
	 FROM EMPLOYEE)
--WHERE ROWNUM BETWEEN 3 AND 7; -- 3행 이상 7행 이하
WHERE ROWNUM BETWEEN 1 AND 7; -- 1행 이상 7행 이하
...
FROM 	
	(SELECT RANK() OVER(ORDER BY SALARY DESC) "순위", EMP_ID, EMP_NAME, PHONE, SALARY, JOB_CODE
	 FROM EMPLOYEE
	 WHERE EXTRACT(YEAR FROM HIRE_DATE) >= 2010)
JOIN JOB USING (JOB_CODE)
WHERE "순위" = 1;
...
WHERE HIRE_DATE = (SELECT MIN(HIRE_DATE)
										FROM EMPLOYEE "SUB"
										WHERE ENT_YN != 'Y'
													AND NVL(SUB.DEPT_CODE, '소속없음') = NVL(MAIN.DEPT_CODE, '소속없음'))
													-- NULL은 비교연상 (=, !=)이 불가능하기 때문에
...
SELECT
	...
	EMP_NAME,
	JOB_NAME,
	FLOOR(MONTHS_BETWEEN(CURRENT_DATE, TO_DATE(SUBSTR(EMP_NO, 1, 6))) / 12) "만 나이",
