...
JOIN TB_DEPARTMENT "TD" ON (TR.DEPARTMENT_NO = TD.DEPARTMENT_NO)
...
WHERE (CLASS_NAME, PROFESSOR_NAME) IN (SELECT CLASS_NAME, PROFESSOR_NAME
									   										...
									   										WHERE DEPARTMENT_NO IN (SELECT DEPARTMENT_NO
											 										   										FROM TB_DEPARTMENT
											 										   										WHERE CATEGORY = '인문사회'))
ORDER BY CLASS_NAME ASC, PROFESSOR_NAME ASC;
...
SELECT STUDENT_NAME "학생 이름", NVL(PROFESSOR_NAME, '지도교수 미지정') "지도교수 이름"
FROM TB_STUDENT
JOIN TB_DEPARTMENT USING (DEPARTMENT_NO)
...
FROM 
	(SELECT RANK() OVER(ORDER BY AVG(POINT) DESC) "순위", STUDENT_NO, STUDENT_NAME
	 FROM TB_STUDENT
	 JOIN TB_GRADE USING (STUDENT_NO)
	 JOIN TB_DEPARTMENT USING(DEPARTMENT_NO)
