...
-- 직원번호, 직원면, 부서코드는 EMPLOYEE 테이블에서 조회가능
...
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
		--> DEPT_CODE와 DEPT_ID가 같은 행을 한 행으로 합쳐라
...
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); -- 21행 (하동운, 이오리 제외)
...
FROM DEPARTMENT
/*INNNER*/ JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE);
...
/* USING 사용 안 하고 JOIN 하기 */
SELECT *
FROM EMPLOYEE
JOIN JOB ON(JOB_CODE = JOB_CODE);
-- ORA-00918 : 열의 정의가 애매합니다
--> JOB_CODE 컬럼명이 같아서 어떤 테이블의 컬럼인지 구분할 수 없기 때문에 오류 발생

	--> 해결 방법 : JOB_CODE 앞에 테이블명 또는 별칭 작성
		-->	EMPLOYEE.JOB_CODE = JOB.JOB_CODE / E.JOB_CODE = J.JOB_CODE;
...
-- 오라클 -> 별칭 사용
-- 테이블 별로 별칭을 등록할 수 있음
SELECT EMP_ID, EMP_NAME, T1.JOB_CODE /* 어떤 테이블의 컬럼을 조회할지 별칭을 작성! */, JOB_NAME
...
FROM EMPLOYEE
/*INNER*/ JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); -- 21행 조회(하동운, 이오리 제외)
...
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); -- 23행 조회(EMPLOYEE 테이블 기준)
...
-- INNER JOIN 결과에 포함되지 않았던 DEPARTMENT의 'D3', 'D4', 'D7'도 JOIN 결과에 포함
...
--> 직접 사용보다는 NATURAL JOIN의 실패 결과로 보여지는 모습이다!
--  라고 알아두는게 좋음
...
FROM EMPLOYEE
NATURAL JOIN JOB;
--> 두 테이블에서 같은 컬럼명, 자료형을 지닌 JOB_CODE 컬럼 기준으로 JOIN이 진행된다
...
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB USING (JOB_CODE)
WHERE EMP_NO LIKE '8%' AND SUBSTR(EMP_NO, 8, 1) = '2' AND EMP_NAME LIKE '전%';
...
--연봉에 보너스포인트를 적용하시오.
SELECT EMP_NAME, JOB_NAME, SALARY, SALARY * 12 * (1+NVL(BONUS, 0)) "연봉"
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
