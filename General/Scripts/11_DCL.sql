 ...
 * 	 접근 권한을 부여(GRANT), 회수(REVOKE)하는 언어
 * */
...
* 관리자 계정 : 데이터베이스의 생성과 관리를 담당하는 계정.
                모든 권한과 책임을 가지는 계정.
                ex) sys(최고관리자), system(sys에서 권한 몇 개 제외된 관리자)
...
SELECT * FROM KH_KHY.EMPLOYEE;

-- 다른 테이블 조회도 되는지 확인
SELECT * FROM KH_KHY.DEPARTMENT;
