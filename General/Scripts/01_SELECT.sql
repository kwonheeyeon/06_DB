...
-- AND(그리고) : 두 조건식의 결과가 모두 TRUE인 경우만 TRUE
	--> 두 조건을 모두 만족하는 행만 조회 결과에 포함

-- OR(또는) : 두 조건식의 결과 중 하나라도 TRUE인 경우에 TRUE
...
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5' OR DEPT_CODE = 'D6' OR DEPT_CODE = 'D9'; -- 12행 조회
...
-- EMPLOYEE 테이블에서
-- 부서 코드가 'D5', 'D6', 'D9'이 아닌 사원의
-- 사번, 이름, 부서 코드 조회 (NULL 미포함)
SELECT EMP_ID, EMP_NAME, DEPT_CODE
FROM EMPLOYEE
WHERE DEPT_CODE NOT IN ('D5', 'D6', 'D9'); -- 9행 조회

-- EMPLOYEE 테이블에서
-- 부서 코드가 'D5', 'D6', 'D9'이 아닌 사원의
-- 사번, 이름, 부서 코드 조회 (NULL 포함)
SELECT EMP_ID, EMP_NAME, DEPT_CODE
FROM EMPLOYEE
WHERE DEPT_CODE NOT IN ('D5', 'D6', 'D9') OR DEPT_CODE IS NULL; -- 11행 조회
...
-- EMPLOYEE 테이블에서
-- EMAIL의 아이디 중 '_' 앞쪽 글자수가 3글자인 사원의
-- 사번, 이름, 이메일 조회
SELECT EMP_ID, EMP_NAME, EMAIL
FROM EMPLOYEE
 ...
 * -> 지정된 특수 문자 뒤 "한 글자"에만 적용된다!!
 * 
 * EX) WHERE 컬럼명 LIKE '__#_' ESACAPE '#';
 *   -> # 뒤에 '_'는 일반 문자열로 인식된다!
 */
