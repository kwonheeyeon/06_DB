...
-- INSTR(문자열 | 컬럼명, '찾을 문자열' [, 찾을 시작위치 [, 순번]])
...
SELECT
  MONTHS_BETWEEN('2024-12-06', '2024-06-24'), -- 5.42 == 5개월 13일
  (TO_DATE('2024-12-06') - TO_DATE('2024-06-24')) / 30 -- 5.5
FROM DUAL;

/* MONTH_BETWEEN이 지정된 두 날짜 사이의 차를 계산할 떄
 * 훨씬 좋다!!(더 정확함)
 * -> 달마다 길이(28, 29, 30, 31)가 다르기 때문에
...
FROM EMPLOYEE
-- WHERE HIRE_DATE BEWTWEEN '2010-01-01' AND '2019-12-31'
...
-- 자릿수 구분(,)
SELECT 123456789, TO_CHAR(123456789, '999,999,999') -- 123,456,789
FROM DUAL;
...
SELECT TO_DATE('24년 8월 16일 금요일 오후 2시 15분 30초', 'YY"년" MM"월" DD"일" DAY PM HH"시" MI"분" SS"초"')
FROM DUAL;
...
-- 모든 사원의 급여 합 조회
SELECT SUM(SALARY) FROM EMPLOYEE; -- 94096240
...
-- 모든 사원의 급여 평균
SELECT FLOOR(AVG(SALARY)) FROM EMPLOYEE; -- 4091140
...
-- EMPLOYEE 테이블에서 전화번호가 등록된 사원의 수
SELECT COUNT(*)
FROM EMPLOYEE
