...
ROLLBACK;
--> COMMIT 되지 않은 DML(INSERT, UPDATAE, DELETE) 취소하기
	-- == DML 수행 전으로 되돌리기
SELECT *
FROM EMPLOYEE2
WHERE EMP_ID = 900;
...
-- '장채현', 900, '001123-4345678', 'jang_ch@kh.or.kr', '01012341234', 'D1', 'J7', 'S3', 4300000
INSERT INTO EMPLOYEE2 (EMP_NAME, EMP_ID, EMP_NO, EMAIL, PHONE, DEPT_CODE, JOB_CODE, SAL_LEVEL, SALARY)
						VALUES('장채현', 900, '001123-4345678', 'jang_ch@kh.or.kr', '01012341234', 'D1', 'J7', 'S3', 4300000);
...
SELECT *
FROM DEPARTMENT2
WHERE DEPT_ID = 'D9'; -- D9 / 전력기획팀 / L1

ROLLBACK;

-- EMPOLYEE2 테이블에서 BONUS를 받지 않는 사원의 
-- BONUS를 0.1로 변경
...
UPDATE EMPLOYEE2
SET (SALARY, BONUS) = (SELECT SALARY, BONUS -- 다중열 서브쿼리 수정
											 FROM EMPLOYEE2
											 WHERE EMP_NAME = '유재식')
WHERE EMP_NAME IN ('노옹철', '전형돈', '정중하', '하동운'); -- 4헹 수정됨
...
-- 테이블에서 지정하는 조건의 값이 존재하면 UPDATE == KEY 값 중복 시 VALUE 수정
-- 조건의 값이 없으면 INSERT됨
...
MERGE INTO EMP_M01 USING EMP_M02 ON(EMP_M01.EMP_ID = EMP_M02.EMP_ID) -- 중복 확인용 KEY
...
WHEN NOT MATCHED THEN -- 중복이 되지 않으면(일치하지 않으면)
INSERT VALUES (EMP_M02.EMP_ID, EMP_M02.EMP_NAME, EMP_M02.EMP_NO, -- 삽입하겠다
