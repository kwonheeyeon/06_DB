...
-- (SYS 관리자 계정 접속) ctrl + 9
ALTER SESSION SET "_ORACLE_SCRIPT" = TRUE;
--> 계정명이 언급되는 상황에서는 위에 구문 반드시 실행!!!

-- VIEW 생성 권한 부여
GRANT CREATE VIEW TO KH_KHY;

-- (KH 계정 접속)
-- VIEW 생성 구문(기초)
CREATE VIEW V_EMP
AS SELECT * FROM EMPLOYEE; -- VIEW 만들어짐
...
-- V_EMP에서 대리 직원들은 이름 오름차순으로 조회
-- (VIEW 조회 결과로 보이는 컬럼명을 이요해야된다!!)
SELECT *
FROM V_EMP
WHERE 직급명 = '대리'
ORDER BY 이름 ASC;
...
-- INSERT 결과 확인 - DEPT_COPY2 원본 TABLE
SELECT * FROM DEPT_COPY2;
-- D0		NULL		L2

-- VIEW에 INSERT를 수행했지만
-- VIEW를 만들 대 사용한 원본 테이블에
-- 값이 INSERT됨을 확인!!
...
-- 왜 사용할까?
--> VIEW를 이용해서 DML(INSERT, UPDATE, DELTE) 하지 말라고!
CREATE OR REPLACE VIEW V_DCOPY2
AS
	SELECT DEPT_ID, LOCATION_ID
	FROM DEPT_COPY2
...
INCREMENT BY 5 -- MEXTVAL 호출 시 5씩 증가
...
SELECT * FROM TB_IDX_TEST WHERE TEST_ID = 'TEST500000'; -- 0.017s

/* 인덱스 0 */
-- TEST_NO가 500000인 행 조회
SELECT * FROM TB_IDX_TEST WHERE TEST_NO = 500000; -- 0.001s
--> 17배 차이 (보통 10 ~ 30배 차이)
