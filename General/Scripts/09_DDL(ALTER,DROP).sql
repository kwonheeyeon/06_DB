...
-- 1) ADD/DROP ???????????????????????????????
ALTER TABLE DEPT_COPY
ADD CONSTRAINT DEPT_COPY_TITLE_NN;
...
-- EX) CHAR(3) 크기의 컬럼에 'ABC' 저장된 상태
-- 			-> CHAR(1) 크기로 수정하려고 하면 오류 발생
...
SELECT DEPT_TITLE, LENGTH(DEPT_TITLE) "문자열 길이" , LENGTHB(DEPT_TITLE) "문자열의 바이트 크기"
FROM DEPT_COPY;
...
-- INSERT(DML 수행)
INSERT INTO DEPT_COPY VALUES('D0'); -- 1행 삽입 
